        ...
        foreach ($this->morphMap as $key => $model) {
            ...
            if($type == $key){
                ...
                if($row = $model::find($id)){
                    ...
                    return array( 'message' => ['error' => "Game of type: $type with id: $id, does not exist in database."], 'status' => 422 );
                }
            }
        }
        // $type does not exist as a valid Morp accessor (if needed, add it to config('morphMap') )
        return array( 'message' => [ 'error' => 'Game of type: '.$type.' does not exist.'], 'status' => 400 );
