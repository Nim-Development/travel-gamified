...
export class NewRoute extends Component {
    ...
    persistTourInfo = () => {
        const body = {
            transit_id: !this.state.transit
                ? null
                : this.state.transit.value.id,
        ...
        axios
            .post(`http://127.0.0.1:8000/api/admin/routes`, body)
            .then(res => {
                console.log(res);
            })
            .catch(err => {
...
if (document.getElementById("new_route")) {
    const element = document.getElementById("new_route");
    const transits = JSON.parse(element.getAttribute("transits"));

    ReactDOM.render(<NewRoute transits={transits} />, element);
}
